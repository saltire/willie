...
def ytget(bot, trigger, uri):
    ...
    try:
        ...
        vid_info['uploaded'] = '%s-%s-%s, %s:%s' % (upraw[0:4], upraw[8:10], upraw[5:7],
...
def ytsearch(bot, trigger):
    ...
    if video_info['link'] == 'N/A':
        ...
        return
    message = ('[YT Search] %s | Length: %s | Uploader: %s | Uploaded: %s | Views: %s | '
               '+%s -%s | Comments: %s | %s'
               % (video_info['title'], video_info['length'], video_info['uploader'],
...
def ytinfo(bot, trigger, found_match=None):
    ...
    message = ('[Youtube] %s | Length: %s | Uploader: %s | Uploaded: %s | Views: %s | '
               '+%s -%s | Comments: %s | %s'
               % (video_info['title'], video_info['length'], video_info['uploader'],
                  video_info['uploaded'], video_info['views'], video_info['likes'],
                  video_info['dislikes'], video_info['comments'], video_info['link']))
...
def ytlast(bot, trigger):
    ...
    message = ('[Latest Video] %s | Length: %s | Uploader: %s | Uploaded: %s | Views: %s | '
               '+%s -%s | Comments: %s | %s'
               % (video_info['title'], video_info['length'], video_info['uploader'],
                  video_info['uploaded'], video_info['views'], video_info['likes'],
                  video_info['dislikes'], video_info['comments'], video_info['link']))
